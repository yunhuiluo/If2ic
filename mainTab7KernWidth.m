...
kernWidth=[300 15];%[10 5 9 13 17 21 25]+20;
for noKernelWidth=1:5
%% options
opt.Nc=5;
...
opt.kernel_b= kernWidth(noKernelWidth);%
opt.kernel_b1=opt.kernel_b;
opt.kernel_b2=opt.kernel_b;
...
for i=1:size(FeatSetNames,2)
    for j=1:size(featNames,2)
        try
            ...
            resultLsq{i,j}.clusterMethod=[featSetName,'(',num2str(opt.kernel_b),')'];
            ...
            save2XlsMethodPerf(resultLsq{i,j},fullfile('results','tab8KernWidth.xlsx'));
        catch
        end
    end
end
